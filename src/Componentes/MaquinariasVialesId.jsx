...
const MaquinariasVialesId = () => {
  ...
  const anteriorImagen = () => {
    setIndex((prevIndex) => (prevIndex - 1 + tractor.imageUrls.length) % tractor.imageUrls.length);
  };
  ...
  if (!tractor) return <p>No se encontró el tractor.</p>;
        ...
        <div className="row">
          ...
          <div className="col-md-6 d-flex flex-column align-self-start">
            <div className="p-4">
              ...
              <h4 className="fw-bold mb-3">{tractor.title}</h4>

              {/* Campos adicionales */}
             {tractor.extraFields && tractor.extraFields.filter(f => f && f.trim() !== "").length > 0 && (
  <div className="mb-4">
    <h6 className="text-muted mb-2">Características:</h6>
    <ul className="list-unstyled ps-2">
      {tractor.extraFields
        .filter(f => f && f.trim() !== "") // solo muestra los que tienen contenido
        .map((field, index) => (
            ...
            <span>{field}</span>
              ...
              {/* Descripción */}
                ...
                <div>
                  ...
                  <p className="text-secondary">{tractor.text}</p>
...
export default MaquinariasVialesId;
